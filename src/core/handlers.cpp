...
bool ensureExactArgs(const std::vector<std::string>& args, std::size_t expected, std::string& out)
{
 ...
 *   - Storage layer failure when adding (file already exists, I/O error, etc.) → 400 BadRequest.
 *
 * Content semantics:
...
std::string handleGet(const std::vector<std::string>& args, Storage& storage)
{
    ...
    // rawTail may contain trailing spaces – we reject any filename with spaces.
 ...
 *
 * Semantics:
 *   - Term is passed exactly as received (after command and space normalization
 *     done by Splitter), so Storage::search can perform exact substring matching
 *     on that term.
 */
std::string handleSearch(const std::vector<std::string>& args, Storage& storage)
{
    ...
    try {
        // Assumes Storage::search(term) returns the same matches string as Ex1.
