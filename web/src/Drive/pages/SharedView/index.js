...
function isSharedWithMe(file, user) {
  if (file?.sharedWithMe === true) return true;
  if (file?.shared === true) return true;

  if (file?.sharedBy) return true;

  const me = user?.username ?? user?.name ?? user?.id ?? "me";
  if (Array.isArray(file?.sharedWith) && file.sharedWith.includes(me)) return true;

  return false;
}

export default function SharedWithMeView({ user }) {
  ...
  const visibleFiles = useMemo(() => {
    const shared = files.filter((f) => isSharedWithMe(f, user));
    return applySort(shared, sortBy, folderPosition);
  }, [files, sortBy, folderPosition, user]);
